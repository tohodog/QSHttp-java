...
import org.qinsong.http.framework.ability.HttpFutureCallback;
import org.qinsong.http.framework.ability.Parser;
...
public class RequestParams {
    ...
    public static final class Builder {
        ...
        public Builder resultByFile(String downloadPath) {
        ...
        //开启由客户端决定的缓存配置
        public Builder clientCache(int timeout) {
            cacheTime(timeout);
         ...
         * 请求键值对参数
         */
        public Builder param(Object object) {
            if (object != null) {
         ...
         * 请求键值对参数
         */
        public Builder param(Map<String, ?> params) {
            if (params != null) {
         ...
         */
        public Builder requestBody(String contentType, Object content) {
         ...
         * 把 params 转为multipartBody参数
         */
        public Builder toMultiBody(String multipartType) {
         ...
         * 上传
         */
        public Builder multipartBody(String key, String contentType, String filename, Object value) {
            toMultiBody();
         ...
         * 上传
         */
        private Builder multipartBody(Map<String, ?> params) {
            toMultiBody();
            if (params != null) {
                for (String key : params.keySet()) {
         ...
         * 返回json解析成对象
         */
        public Builder jsonModel(Class<?> _class) {
            this._class = _class;
         ...
         */
        @Deprecated
        public Builder xmlModel(Class<?> _class) {
            this._class = _class;
         ...
         */
        public Builder qsClient(String qsClient) {
         ...
         * 暂时没用
         */
        private Builder cacheTime(int cacheTime) {
            this.cacheTime = cacheTime;
            return this;
        }

        //构建并执行,异步
        public int asyncExecute(HttpCallback cb) {
            return build().execute(cb);
        }

        //构建并执行,异步Future
        public Future<ResponseParams> futureExecute() {
            return build().futureExecute();
        }

        //构建并执行,同步(这里不用futureExecute().get(),因为线程切换会有性能损耗,重新写个)
        public ResponseParams syncExecute() throws Exception {
    ...
    public int execute(HttpCallback cb) {
        if (qsClient == null)
            ...
            if (qsHttpClient == null) {
                Log.e("can't find client:" + qsClient);
                cb.onFailure(HttpException.Custom("can't find client:" + qsClient));
                return -1;
            }
